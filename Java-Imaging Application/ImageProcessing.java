...
public class ImageProcessing extends JFrame implements ActionListener
{
	...
	JMenuBar jmb;
	JMenu m_File,m_Arith,m_Filter,m_Enhance,m_overlap;
	JMenuItem m_Open,m_Save,m_Exit,m_open1;
	...
	/************* Initailization of every component and variable being used **************/
	ImageProcessing()
	{
	...
	void OpenImage2() 
	{
		int returnVal = jfc.showOpenDialog(ImageProcessing.this);
	...
	/******************* Convert pixels to 32 bit format for Display *************/
...
/********************** Mask Operation for Filters and Edge Detectors (Convolution) ********************/
 ...
 int[][] MaskOperation(int width,int height,int pix_tempImg[][], int temp_mask[][],int d)
 {
	...
	for(int i =1;i<height-1;i++)
	{
		for(int j=1;j<width-1;j++)
		{
			pix_tempImg1[i][j] = ((((temp_mask[0][0]*pix_tempImg[i-1][j-1]) + (temp_mask[1][0]*pix_tempImg[i][j-1]) + (temp_mask[2][0]*pix_tempImg[i+1][j-1]))+
								...
								((temp_mask[0][2]*pix_tempImg[i-1][j+1]) + (temp_mask[1][2]*pix_tempImg[i][j+1]) + (temp_mask[2][2]*pix_tempImg[i+1][j+1])))/d);
		}
...
int [][] mean_filt(int pixell_2d[][])
{
	...
	public void actionPerformed(ActionEvent e)
	{
       ...
       /************************* Multiply Constant value and then use Satuartion technique ****************/ 		
    	...
    	if(e.getSource() == mDivImg)
		{
     	...
     	/************************* Divide by Constant value and then use Satuartion technique ****************/
       ...
       /************** Divide constant by Wrap Around technique ***********/
   		...
   		if(e.getSource() == mThreshold)
        {
       	...
       	if(e.getSource() ==m_Save)
   		{
   				...
   				int returnVal = jfc.showSaveDialog(ImageProcessing.this);
				if (returnVal == JFileChooser.APPROVE_OPTION)
				{
       ...
       /******************** Contrast Strtching ****************/
         ...
         if(e.getSource() == mContrast)
        {
        	...
        	for(int r = 0; r < w*h ; r++)
        	{
        		if((pixel_result[r] <= number1) && (pixel_result[r] >= number))
        		{
   	 ...
   	 /********************* Low Pass Filter *******************/	
   		...
   		if(e.getSource() == mhighPass)
   		{
   			...
   			pix_tempLow= MaskOperation(w,h,pix_tempLow,mask,35);
   			int pix_temp1D[] = new int[(w)*(h)];
   		...
   		//	pixx =  WrapAround(pix_temp1D,w-2,h-2);
		...
		if(arg.equals("Prewits Horizontal"))
		{
			...
			pix_grabed = pixel_grab(image,w,h);
			p2d = OneD_ArrayToTwoD_Array(w,h,pix_grabed);
			pp2d= MaskOperation(w,h,p2d,mask,75);
		...
		if(arg.equals("Prewits Both"))
		{
			...
			p2d = OneD_ArrayToTwoD_Array(w,h,pix_grabed);
			pp2d= MaskOperation(w,h,p2d,mask1,75);
			pres1 = TwoD_ArrayToOneD_Array(w,h,pp2d); 
			
			int mask2[][] = {{-1,0,1},{-1,0,1},{-1,0,1}};
			p2d = OneD_ArrayToTwoD_Array(w,h,pix_grabed);
			pp2d= MaskOperation(w,h,p2d,mask2,75);
			...
			for(int i=1;i<h-1;i++)
			{
				for(int j =1;j<w-1;j++)
				{
					pp2d[i][j] = ((mask[0][0]*p2d[i-1][j-1] + mask[1][0]*p2d[i+1][j-1])+
								  (mask[0][1]*p2d[i-1][j+1] + mask[1][1]*p2d[i+1][j+1]))/255;
				}
	...
	/************************ Laplacian Edge Detector ********************/
		...
		if(arg.equals("Laplacian"))
		{
			...
			pix_grabed = pixel_grab(image,w,h);
			p2d = OneD_ArrayToTwoD_Array(w,h,pix_grabed);
			pp2d= MaskOperation(w,h,p2d,mask,100);
